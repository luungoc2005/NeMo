...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.intermediate_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.intermediate_size)

        self.embedding_proj = nn.Linear(config.hidden_size, config.intermediate_size)
    ...
    def __init__(self, config):
        ...
        if config.intermediate_size % config.num_attention_heads != 0:
            raise ValueError(
                ...
                "heads (%d)" % (config.intermediate_size, config.num_attention_heads))
        ...
        self.query = nn.Linear(config.intermediate_size, self.all_head_size)
        self.key = nn.Linear(config.intermediate_size, self.all_head_size)
        self.value = nn.Linear(config.intermediate_size, self.all_head_size)
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.intermediate_size, config.feedforward_size)
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([layer for _ in range(config.num_hidden_layers)])
